...
def page_4():
    ...
    if not st.session_state.get("feedback_saved", False):
        ...
        prompt += "위 대화를 바탕으로, 다음 내용을 포함해 탐구 내용과 피드백을 작성하세요: 1. 대화 내용을 종합해 도출한 탐구 가설 및 과정, 2. 학생이 제시한 탐구 가설 및 과정에서 수정한 부분과 수정한 이유, 3. 학생의 탐구 능력에 관한 피드백(강점과 개선점 등), 4. 예상 결과(주제와 관련된 과학적 이론과 실험 오차를 고려해, 실험 과정을 그대로 수행했을 때 나올 실험 결과를 표 등으로 제시해주세요. 이때 결과 관련 설명은 제시하지 말고, 결과만 제시하세요). 만약 학생이 충분한 대화를 하지 않고 이 페이지로 넘어왔다면, 이전 버튼을 눌러 실험에 대해 인공지능과 더 대화를 해야 한다고 안내하세요."

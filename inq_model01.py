...
def page_4():
    ...
    if "experiment_plan" not in st.session_state or not st.session_state.get("feedback_saved", False):
        ...
        st.session_state["feedback_saved"] = False  # 저장 여부 초기화

    # 피드백 출력 및 저장 처리
    if "experiment_plan" in st.session_state and not st.session_state.get("feedback_saved", False):
        if save_feedback_to_db(st.session_state["experiment_plan"]):  # DB 저장 시도
            st.session_state["feedback_saved"] = True  # 저장 성공 플래그 설정
            st.success("피드백이 성공적으로 저장되었습니다.")
        else:
